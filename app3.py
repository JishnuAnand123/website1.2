...
def create_table():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS final_user_data (
            ...
            additional_link VARCHAR(100) DEFAULT 'NONE',                        
...
def insert_data(name, phone_num, email,address,location,github,linkdin,additional_link, job_role, file_name,type):
    conn = psycopg2.connect(host=db_host, user=db_user, password=db_password, dbname=db_name)
    cur = conn.cursor()
    cur.execute("INSERT INTO final_user_data (name, phone_num, email,address,location,github,linkdin,additional_link,job_role, file_name,type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
